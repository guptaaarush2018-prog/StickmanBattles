...
const CHANGELOG = [
  {
    ...
    changes: [
      ...
      { cat: 'Mode',    text: 'God fight has two phases — Phase 1 is a one-sided encounter where God has 100,000 HP and the player is not expected to win; Phase 2 unlocks after a prior encounter or Sovereign defeat, grants the player the Godslayer weapon and spawns Paradox as an ally who fights God alongside you' },
      { cat: 'Mode',    text: 'Added Godslayer — a heavy melee weapon with 180 base damage and the Divine Strike ability (lunge + 380 damage burst); automatically equipped to P1 when a God fight begins' },
      { cat: 'Entity',  text: 'God is a white stickman with a glowing halo; it tracks the nearest player, enforces a hard 160px attack range gate at the combat system level, and has a separate speed and attack cadence per phase' },
      { cat: 'Entity',  text: 'Added GodParadoxAlly — Paradox appears as a playable-style ally in Phase 2, locks onto God exclusively, and fights autonomously with its own attack cooldown and health bar' },
      { cat: 'Combat',  text: 'Added fake crash screen — when God lands its first hit on a human player in Phase 1, the game displays a convincing browser crash overlay; resolves on click and resumes match; suppressed when God is spawned via dev console' },
      { cat: 'Combat',  text: 'God hard range gate added to dealDamage() — God attacks skip the damage pipeline entirely if the target is more than 160px away; enforced at the system level above all AI-layer decisions' },
      ...
      { cat: 'Debug',   text: 'Added spawn/summon god console commands — spawns God without the fake crash screen for dev testing; God is a singleton and cannot be double-spawned' },
      { cat: 'Achievement', text: 'Added "God Slayer" achievement — awarded on defeating God in Phase 2' },
    ],
  },
  {
    version: '2.8.0',
